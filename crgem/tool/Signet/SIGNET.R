...
Genesets <- function(copaired){
  genesets_list <- levels(as.factor(copaired$V1))  #--> sorted unique list of TFs
  # genesets= as.data.frame(matrix(nrow=length(genesets_list),ncol=1))
  valid_genesets_idx <- c()
  count = 1
  for (i in 1:length(genesets_list)){
    # genesets[i,1] = paste(copaired[copaired$V1==genesets_list[i],2],collapse = ",")
    if (length(copaired[copaired$V1==genesets_list[i],2]) > 1){
      valid_genesets_idx[count] <- i
      print(count)
      count = count+1
    }

  }
  genesets= as.data.frame(matrix(nrow=length(valid_genesets_idx),ncol=1))
  valid_genesets_list <- c()
  for (i in 1:length(valid_genesets_idx)){
    genesets[i,1] = paste(copaired[copaired$V1==genesets_list[valid_genesets_idx[i]],2],collapse = ",")
    valid_genesets_list[i] <- genesets_list[valid_genesets_idx[i]]
  }
  #rownames(genesets)=genesets_list
  rownames(genesets)=valid_genesets_list
