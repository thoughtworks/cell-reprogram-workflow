...
def generate_network(node_data, trrust_analysis_data, artefacts_path):
    ...
    p4c.export_network(artefacts_path+"/Cytoscape/"+'network_from_trrust_output', type='SIF') 
...
def preprocess_sif(artefacts_path):
    ...
    preprocessed_file = p4c.export_network(path + 'preprocessed_' + network_file, type='SIF')
    print(preprocessed_file)

def call_api(params,network_file):
    print(f'Sources (TFs): {params["sources"]}\nTargets (Marker + Target genes: {params["targets"]}')
    params["k"] = 100
...
def get_regulators(TFs, targets, artefacts_path):
    ...
    p4c.export_network(filename =  artefacts_path+"/Cytoscape/" + 'PathLinker_TF_Targets_Paths.sif', type='SIF')
...
def get_master_regulators(TFs,artefacts_path):
    ...
    print(len(master_raw))
    p4c.clear_selection()
    params = {}
    params['sources'] = master_raw
    params['targets'] = TFs
    call_api(params, network_file)
    p4c.export_network(filename = artefacts_path+"/Cytoscape/"+'PathLinker_MasterRegulators_TFs_Paths.sif', type='SIF')
...
def merge_networks(artefacts_path):
    ...
    try:
        ...
        print(f'Error thrown by p4cytoscape during merging of networks. The merged network is created in Cytoscape though!')
    p4c.export_network(filename =  artefacts_path+ "/Cytoscape/MasterRegulators_TFs_Targets_Merged_Network" , type='SIF')
...
def set_visual_style(merged_network_file, TFs, targets,  artefacts_path):
        ...
        p4c.export_network(filename =  artefacts_path+ "/Cytoscape/MasterRegulators_TFs_Targets_Network" , type='SIF')
        
 
def create_network(trrust_output,artefacts_path):
    print("RUNNING generate_network with params", trrust_output)
    merged_network_file = "MasterRegulators_TFs_Targets_Merged_Network.sif"
