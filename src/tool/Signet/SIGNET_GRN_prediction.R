...
library(RcisTarget)
getwd()
source(paste(code_path,'/SIGNET.R',sep=""))
...
motifRankings <- importRankings(paste(code_path,"/hg19-tss-centered-10kb-10species.mc9nr.feather",sep=""))
...
write.csv(copaired, (paste(in_path,"/copaired1.csv",sep="")))
genesets <- Genesets(copaired)
write.csv(genesets,(paste(in_path,"/genesets1.csv",sep="")))
genesets_list <- levels(as.factor(copaired$V1))
write.csv(genesets_list,(paste(in_path,"/genesets_list1.csv",sep="")))
...
write.csv(copaired2,(paste(in_path,"/copaired2.csv",sep="")))

genesets2 <- Genesets(copaired2)
write.csv(genesets2,(paste(in_path,"/genesets2.csv",sep="")))

genesets_list2 <- levels(as.factor(copaired2$V1))
write.csv(genesets_list2,(paste(in_path,"/genesets_list2.csv",sep="")))
